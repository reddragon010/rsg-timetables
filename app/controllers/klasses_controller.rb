class KlassesController < ApplicationController
  ...
  def show
    ...
    #raise @klass.entries.inspect
    @tt_rows = Array.new
    #raise @tt_rows.inspect
    (1..16).each do |i|
      entries = @klass.entries.map{|e| e if e.lession == i}.compact
      schooldays = ["monday","thuesday","wendnesday","thursday","friday","saturday","sunday"]
      @tt_rows[i] = Hash.new
      @tt_rows[i]["lession"] = i
      @tt_rows[i]["days"] = Array.new
      schooldays.each_with_index do |day,di|
        @tt_rows[i]["days"] << entries.map{|e| {:subject => e.subject.short, :teachers => e.teachers.map{|t| t.short}, :rooms => e.rooms.map{|r| r.short}} if e.schoolday == day}.compact
      end
    end
    #raise @tt_rows.inspect
